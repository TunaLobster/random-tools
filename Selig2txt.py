...
parser = argparse.ArgumentParser(description='Convert a Selig or xfoil airfoil file to .txt to be used in Solidworks')
parser.add_argument('inputfile', help='Path to file to be used as input. If it is in the same directory, just the filename.')
parser.add_argument('--split', help='Split the airfoil into two curves. Two output files are created', action='store_true')
...
if __name__ == "__main__":
    try:
        ...
        contents = re.sub(match1, '', contents)  # remove leading whitespace on all the lines
        ...
        contents = re.sub(match4, lambda match: f'{match.group(1)},0.000000\n', contents)
